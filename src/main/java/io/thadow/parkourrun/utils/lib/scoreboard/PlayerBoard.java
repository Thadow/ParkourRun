...
public class PlayerBoard implements IPlayerBoard<String, Integer, String> {
    ...
    private void sendObjective(Objective obj, ObjectiveMode mode) {
        try {
            Object objHandle = Reflection.getHandle(obj);

            Object packetObj = Reflection.PACKET_OBJ.newInstance(
                    objHandle,
                    mode.ordinal()
            );

            Reflection.sendPacket(packetObj, player);
    ...
    private void sendObjectiveDisplay(Objective obj) {
        try {
            ...
            Object packet = Reflection.PACKET_DISPLAY.newInstance(
                    1,
                    objHandle
            );
    ...
    private void sendScore(Objective obj, String name, int score, boolean remove) {
        try {
            Object sbHandle = Reflection.getHandle(scoreboard);
            Object objHandle = Reflection.getHandle(obj);

            Object sbScore = Reflection.SB_SCORE.newInstance(
                    sbHandle,
                    objHandle,
                    name
            );
            ...
            switch(Reflection.getVersion().getMajor()) {
                case "1.7": {
                    Object packet = Reflection.PACKET_SCORE.newInstance(
                            sbScore,
                            remove ? 1 : 0
                    );
                ...
                case "1.12": {
                    ...
                    if(remove) {
                        packet = Reflection.PACKET_SCORE_REMOVE.newInstance(
                                name,
                                objHandle
                        );
                    }
                    else {
                        packet = Reflection.PACKET_SCORE.newInstance(
                                sbScore
                        );
                    }
                ...
                default: {
                    Object packet = Reflection.PACKET_SCORE.newInstance(
                            remove ? Reflection.ENUM_SCORE_ACTION_REMOVE : Reflection.ENUM_SCORE_ACTION_CHANGE,
                            obj.getName(),
                            name,
                            score
                    );
    ...
    public void delete() {
        ...
        io.thadow.parkourrun.utils.lib.scoreboard.Scoreboard.instance().removeBoard(player);
