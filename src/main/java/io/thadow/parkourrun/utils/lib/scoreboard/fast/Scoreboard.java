package io.thadow.parkourrun.utils.lib.scoreboard.fast;
...
public class Scoreboard {
    public static final Map<UUID, io.thadow.parkourrun.utils.lib.scoreboard.fast.Scoreboard> scoreboards = new HashMap<>();
    ...
    private static void updateLobbyScoreboard(Player player) {
        ...
        if (arena == null) {
            ...
            io.thadow.parkourrun.utils.lib.scoreboard.fast.Scoreboard scoreboard = scoreboards.get(player.getUniqueId());
            if (scoreboard == null) {
                scoreboard = new io.thadow.parkourrun.utils.lib.scoreboard.fast.Scoreboard(player);
                scoreboards.put(player.getUniqueId(), scoreboard);
            }
            scoreboard.updateTitle(Utils.colorize(title));
            for (String line : lines) {
                line = Utils.replace(line, "%player_name%", player.getName());
                line = Utils.replace(line, "%wins%", PlayerDataManager.getPlayerDataManager().getPlayerWins(player).toString());
                line = Utils.replace(line, "%loses%", PlayerDataManager.getPlayerDataManager().getPlayerLoses(player).toString());
